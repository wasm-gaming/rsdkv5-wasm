...
# Builds Sonic Mania (RSDKv5U decompilation) as WASM — APPROACH B: SDK contract
# (createRSDKv5 ES6 factory + runtime data mount). See
# docs/wasm-build-approach-b-sdk-contract.md. The engine-porting core (clone,
# vendored deps, the four engine patches) is IDENTICAL to the proven recipe
# (.peers/build-rsdkv5-docker.sh, play.germade, verified 2026-07-05 through
# Studiopolis gameplay); only the link flags and data handling diverge:
# MODULARIZE/EXPORT_ES6/EXPORT_NAME=createRSDKv5, INVOKE_RUN=0, and NO
# --preload-file — the JS SDK (src/rsdkv5.sdk.ts) writes Data.rsdk/Settings.ini
# into the FS at runtime, then calls Module.callMain().
#
...
#
# The game is still compiled STATICALLY into the engine (-DGAME_STATIC=ON): the
# .wasm is Sonic-Mania-specific; only the SDK surface is game-agnostic. No game
# data is needed at build time. Single-threaded → no COOP/COEP → Pages-hostable.
#
...
target_link_options(RetroEngine PRIVATE
    ...
    -sSTACK_SIZE=5242880
    -sFORCE_FILESYSTEM=1
    # --- APPROACH B divergence from the proven flags (everything above is
    # identical). ES6 factory + deferred boot, so src/rsdkv5.sdk.ts can do
    # `const Module = await createRSDKv5(...)`, write Data.rsdk/Settings.ini
    # into the FS, then Module.callMain([]). No --preload-file. ---
    -sMODULARIZE=1
    -sEXPORT_ES6=1
...
if [[ -z "$ENGINE_JS" ]]; then
  ...
  echo "  not have taken effect. Emitted js/wasm files:" >&2
  find "$WORK_DIR/build" \( -name '*.js' -o -name '*.wasm' \) -not -path '*CMakeFiles*' >&2 || true
  exit 1
fi
ENGINE_BUILD_DIR="$(dirname "$ENGINE_JS")"
# No .data artifact in approach B: game data is mounted at runtime by the SDK.
cp "$ENGINE_BUILD_DIR/rsdkv5.js" "$ENGINE_BUILD_DIR/rsdkv5.wasm" "$DIST_DIR/rsdkv5/"
